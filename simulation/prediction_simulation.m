...
On_center = 4*gaussmf(range,[sigma*3 stimulus_range/2]);
Off_surround = -1*gaussmf(range,[sigma stimulus_range/2]);
% figure(1)
% plot(range,On_center);hold on
% plot(range,Off_surround);hold off
% figure(2)
% plot(range,On_center+Off_surround);
...
Off_temporal(9:9+size_curve-1) = Off_temporal(5:5+size_curve-1)+curve;
figure(3)
...
conv_off = zeros(1,T);
for t = 1:T
    conv_on(t) = sum(bar_position(t,:).*On_center);
    conv_off(t) = sum(bar_position(t,:).*Off_surround);
end
% figure(4)
% plot(1:T,conv_on);hold on
% plot(1:T,conv_off);hold on
% plot(1:T,normalize(newXarray),'k.');hold off
% xlabel('time')
...
ratio_on = 1;
ratio_off = 2;
Subtraction = ratio_on*temp_on + ratio_off*temp_off;
% figure(7)
