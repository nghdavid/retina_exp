function [former_name,post_name,filename] = Get_HMM_OU_name(exp_folder,stimulus,type,order,frequency,dark)
cd(exp_folder)
load('different_G.mat')
if dark 
    if strcmp(stimulus,'HMM')
    former_name = [type,'_',HMM_date1,'_',stimulus,'_Dark_',direction,'_G'];
    if order == '0'
        post_name = ['_',num2str(mins),'min_Q100_',num2str(mean_lumin),'mW_100'];
    else
        post_name = ['_',num2str(mins),'min_Q100_',num2str(mean_lumin),'mW_100_',order];
    end
    filename =  [type,'_',HMM_date1,'_',stimulus,'_Dark_',direction,'_',num2str(length(HMM_different_G )),'G_',num2str(mins),'min_Q100_100_',order,'_ch']; %Filename used to save(Plot HMM only)   
elseif strcmp(stimulus,'OUsmooth')
     former_name = [type,'_',OU_date,'_',stimulus,'_Dark_',direction,'_G'];
     post_name = ['_',num2str(mins),'min_Q100_',num2str(mean_lumin),'mW_100_',num2str(frequency),'Hz'];
     filename =  [type,'_',OU_date,'_',stimulus,'_Dark_',direction,'_',num2str(length(OUsmooth_different_G )),'G_',num2str(mins),'min_Q100_100_',num2str(frequency),'Hz_',order,'_ch']; %Filename used to save(Plot OU only)   
elseif strcmp(stimulus,'OU')
    former_name = [type,'_',OU_date,'_',stimulus,'_Dark_',direction,'_G'];
    post_name = ['_',num2str(mins),'min_Q100_',num2str(mean_lumin),'mW_100'];
    filename =  [type,'_',OU_date,'_',stimulus,'_Dark_',direction,'_',num2str(length(OU_different_G )),'G_',num2str(mins),'min_Q100_100_',order,'_ch']; %Filename used to save(Plot smooth only)
end
