function [reconstrFrames, reconstrStimBins, filter] = getLinearPopulationReadoutTimeShift(spikeCounts, stimFrames, filterLen, binshift)
...
%   Apply timeshift
if binshift >= 0
    stimFrames = stimFrames(:, 1+binshift:end);
    spikeCounts = spikeCounts(:,1:end-binshift);
else
    binshift = abs(binshift);
    stimFrames = stimFrames(:, 1:end-binshift);
    spikeCounts = spikeCounts(:,1+binshift:end);
end
