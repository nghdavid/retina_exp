...
% rdrawhd - parses MCS data header (version 1) (datastream method)
...
%	disp(['data section should start at ' int2str(offset_to_data) ' byte']);
   ...
   if word>30
	...
	if ~isempty(word)
		...
		str   = rot90(setstr(str));      	     	  	% header class name. "CMRawDataHeaderInfo"
	end
	switch str
...
%----- parseChannelFilter -------------------------
	...
	str  = rot90(setstr(str));				% channel filter class name. "CChannelFilter"
...
%----- parseTriggerProperty -------------------------
   ...
   disp(['pretriggerticks = ' num2str(pretriggerticks)]);	
	...
	if(strcmp(d.DataType,'raw continous'))
	   ...
	   disp(['total_window_ms = ' num2str(int)]);
...
%	disp(['filter window width = ' num2str(filter_width_us) ' us'])
...
% -------- read rest of file header specific for analog data files ------------------------------
	...
	disp(['this file contains ' d.DataType])
