function sN_indexnCentre = Spatial_Noise_generator2(mea_range, num_dot, monitor_dot_lumin);
...
if num_dot == 1
    ...
    sN_indexnCentre{1}((sN_indexnCentre{2}(1)-16:sN_indexnCentre{2}(1)+16) , (sN_indexnCentre{2}(2)-16:sN_indexnCentre{2}(2)+16)) = monitor_dot_lumin;
    return
else
    sN_indexnCentre = Spatial_Noise_generator2(mea_range, num_dot-1, monitor_dot_lumin);
    if ~isempty(sN_indexnCentre)
        ...
        if length(part_pos) <= 0 %if there is no place for new dot
            ...
            sN_indexnCentre{1}((new_dot_center(1)-16:new_dot_center(1)+16) , (new_dot_center(2)-16:new_dot_center(2)+16)) = monitor_dot_lumin;
        end
    end
end
end