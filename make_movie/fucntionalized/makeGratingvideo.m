function makeGratingvideo(makemovie_folder,video_folder, videoworkspace_folder, date,calibration_date, background_lumin, bar_lumin)
...
OLED_background_lumin_lumin = interp1(real_lum,lum,background_lumin,'linear');
background = zeros(1024,1280);
background(leftx_bd:rightx_bd, lefty_bd:righty_bd) = OLED_background_lumin;
Y =meaCenter_y;
cd (video_folder)
%% video setting
name=[date,'_Grating'];
...
while isempty(SET_I)
    ...
    if theta2pi >= pi, theta = theta2pi-pi; end
    ...
    xarray(1,1) = randi(mea_size_bm)-1;%leftx_bd+bar_wid+1;%Left bar first position %randomly 0~mea_size_bm-1
    for i = 2:num_move
