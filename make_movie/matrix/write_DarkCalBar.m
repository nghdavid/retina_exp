function a = write_DarkCalBar(a, Vertex, theta,  mea_size_bm,calibration_date,monitor_dark_lum)
%Rotation is degree that bar rotate
%monitor_dark_lum is luminance of dark bar
...
if theta == 0 || theta == pi/2  % vertical case
    ...
    for x = floor(min_x) : ceil(max_x)
        for y = floor(lower_y) : ceil(upper_y)
            a(y+lefty_bd-1,x+leftx_bd-1) = monitor_dark_lum;
        end
    end
