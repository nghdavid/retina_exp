function detect_point = find_detect_pt(img,N,sensitivity,radius_range,show,ideal_point)
    ...
    while detected_num ~= N^2%Stay in loop and increase sensitivity to find correct number of points
        ...
        centers( find(centers(:,1)> max(ideal_point(1,:))+15 ),:) = [];
        centers( find(centers(:,2)> max(ideal_point(2,:))+15 ),:) = [];
        centers( find(centers(:,1)< min(ideal_point(1,:))-15 ),:) = [];
        centers( find(centers(:,2)< min(ideal_point(2,:))-15 ),:) = [];
        %Show image
        if strcmp(show,'on')
            figure;imshow(CI); 
