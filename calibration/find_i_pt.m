function [ideal_pt,ideal_distance_pt] = find_i_pt(input_N,cal_size)
    ...
    load('dot7.mat'); %read in the ccd image
    ...
    c1 = detect_pt{floor(N/2)  ,floor(N/2)  };   %c2 c7 c4
    c2 = detect_pt{floor(N/2)+2,floor(N/2)  };   %c5    c8
    c3 = detect_pt{floor(N/2)  ,floor(N/2)+2};   %c1 c6 c3
    ...
    ideal_distance_pt = (norm(c1-c5) + norm(c5-c2) + norm(c6-c9) + norm(c9-c7) + norm(c3-c8) + norm(c8-c4) + norm(c1-c6) + norm(c5-c9) + norm(c6-c3) + norm(c9-c8) + norm(c2-c7) + norm(c7-c4))/12/pt_distance_per_pixel;
    for r=1:cal_size
        for c=1:cal_size
            ideal_pt{r,c} = center + [(c-round(cal_size/2))*ideal_distance_pt , (r-round(cal_size/2))*ideal_distance_pt ];
        end
    end
